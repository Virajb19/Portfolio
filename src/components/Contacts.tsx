...
export default function Contacts() {
  ...
  return <footer id="contact" className="flex flex-col gap-10 items-center mt-4">
          ...
          <div className="flex items-center gap-5 mb:gap-3">
              {Links.map((link, i) => {
                 return <motion.div key={i} initial={{opacity: 0, y: 15, rotate: 20}} whileInView={{opacity: 1, y: 0, rotate: 0}} transition={{duration: 1, delay: i * 0.2, type: 'spring', bounce: 0.5}} 
                 className="flex flex-col items-center gap-1">
                      ...
                      <link.icon className="size-12 mb:size-7 text-orange-600 group-hover:-translate-y-1 group-hover:rotate-6 group-hover:scale-105 duration-300"/>
                  </Link>
